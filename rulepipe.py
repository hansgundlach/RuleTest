...
for i in range(num_examples):
    output = ask_openai(
        prompt + random_string_from_file("tinyshak.txt", length=100) + "(XYZ)"
    )
    results.append(output)
print(results)
...
question = """Think step by step about the rule behind the labels?"""
...
COTprompt = (
    ...
    + "Given your initial guess to the rule, what is your final answer for the general rule behind the labels?"
)
